...
class Moving_Enemy(Enemy):
    def __init__(self, container, ypos, xpos):
        Enemy.__init__(self, container, ypos, xpos)
        self.image = pygame.image.load("Image/moving_enemy.png").convert_alpha()
        self.image = pygame.transform.scale(self.image, (50, 30))
        self.move = False

    def update(self, bullet_group, ship, enemy_group, platform_group, heal_group, camera):
        Enemy.update(self, bullet_group, ship, enemy_group, platform_group, heal_group, camera)
...
class Sniper(Enemy):
    def __init__(self, container, ypos, xpos):
        Enemy.__init__(self,container, ypos, xpos)
        self.image = pygame.image.load("Image/sniper.png").convert_alpha()
        self.image = pygame.transform.scale(self.image, (50,30))
...
class DaBoss(Enemy):
    ...
    def update(self, bullet_group, ship, enemy_group, platform_group, power_up_group, camera):
        Enemy.update(self, bullet_group, ship, enemy_group, platform_group, power_up_group, camera)

    def collide(self, bullet_group, enemy_group, platform_group, dir):
        n = 1
        for b in bullet_group:
            if pygame.sprite.collide_rect(self, b):
                if self.rect.bottom + 100 <= b.rect.top:
                    self.life -= 1
                    b.kill()
